...
$now_db = 'std_listname';
$header = get_table_header("std_listname");
function db_getheader_type(){
    $arr = [];
    $sql = "SHOW COLUMNS FROM ".$GLOBALS['db']['name'].".std_listname;";
...
if(checkValidKey($_POST, $header)){
    ...
    if($r == sizeof($header)){
            ...
            if (in_array($infopath, $GLOBALS['img']['support'])){
                ...
                if (move_uploaded_file($_FILES["image"]["tmp_name"], $img_path.$encode_basename)) {
                    ...
                    if(db_connect()->query($sql)){
                        $sql = "SELECT * FROM std_img_list WHERE sid='".$_POST['sid']."'";
                        ...
                        $sql = "UPDATE std_img_list SET url='".$encode_basename."' WHERE sid='".$_POST['sid']."'";
                        $res = db_connect()->query($sql);
                    }
        ...
        header("refresh:1;url=../?p=edit_student");
    }else{
...
<form action="ed_std_profile.php" class="form-horizontal" method="post" enctype="multipart/form-data">
    <?php
