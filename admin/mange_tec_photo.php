...
$imgPower = $imgPower->get_list_images("teacher");
$isFixPhotos = false;
$img_root = "../teacher_img/";
...
$menu = [
    ...
    1 => [
        'title' => 'จัดการ',
        'url' => '?p=edit_teacher&q=rm'
    ],
    2 => [
        'title' => 'เพิ่มทีละหลายภาพ',
        'url' => '?p=edit_teacher&q=ma'
    ],
];
...
function drop($sid, $file){
    ...
    $sql = "DELETE FROM tec_img_list WHERE url='".$sid."'";
    db_connect()->query($sql);
    header("Refresh:0; url='?p=edit_teacher&q=rm'");
}
...
foreach ($imgPower as $d) {
    ...
    array_push($arr_img,
        ["<img src=\"../teacher_img/" . $d['url'] . "\"  width=180px>",
            $d['sid'],
            "<a href=\"".$onlyPath."/view/ed_tec_profile.php?id=".$d['sid'] . "\">แก้ไข</a>",
            "<a href=\"?p=edit_teacher&q=rm&id=" . $d['url'] . "\">ลบ</a>"
        ]
    );
}
...
/* Close multi Upload */
//$addPhotoa = new AddPhoto("teacher",'../teacher_img/');
...
<div>
    ...
    if(isset($_GET['q']) && $_GET['q']=='ma'){
        include_once dirname(__FILE__)."/view/multi_upload_tec.php";
    }else{
    ?>
        <table>
